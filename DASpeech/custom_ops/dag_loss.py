...
class DagLossWithAlphaBetaFunc(Function):
    config = 1
    config1 = 2
    config2 = 2
    ...
    def forward(
        ...
        Output:
            res (torch.FloatTensor or torch.HalfTensor):
                Shape: [batch_size]
                the loss of each sample
        ...
        match_all = match_all.contiguous()
        links = links.contiguous()
        alpha, beta = get_dag_kernel().dag_loss(match_all, links, output_length, target_length, require_gradient, DagLossWithAlphaBetaFunc.config) # bsz * prelen * tarlen
    ...
    @staticmethod
    def backward(ctx, grad_output, unused):
        alpha, beta, match_all, links, output_length, target_length = ctx.saved_tensors
        if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
            grad_match_all, grad_links = get_dag_kernel().dag_loss_backward(grad_output, alpha, beta, match_all, links, output_length, target_length, DagLossWithAlphaBetaFunc.config1, DagLossWithAlphaBetaFunc.config2)
            return grad_match_all, grad_links, None, None
        else:
            return None, None, None, None

dag_loss = DagLossFunc.apply
dag_loss_with_alpha_beta = DagLossWithAlphaBetaFunc.apply
